            ...
            if (is) { // 如果输入流正常
                ...
                do {
                    if (str.m_size + 1 > str.m_capacity) {
                        str.m_data[str.m_capacity] = '\0';// 如果空间不足 那么扩容
...
struct Person {
    ...
    Person(my::string id, my::string name, my::string address) : id(id), name(name), address(address) {}
...
int main() {
    ...
    std::ifstream in1("ID5000000.in"), in2("ID5000000_s.in");
    std::ofstream out("5000000.out");
    my::BinaryTree<Person> s;
    Person p;
    while (in1 >> p.id >> p.name >> p.address) {
        s.insert(p);
    }
    my::string id;
    while (in2 >> id) {
        auto it = s.find(Person(id, my::string(),  my::string()));
        if (it != nullptr) {
            out << id << " " << it->data.name << " " << it->data.address << "\n";
        } else {
            out << id << " " << "查无此人" << "\n";
        }
    }
